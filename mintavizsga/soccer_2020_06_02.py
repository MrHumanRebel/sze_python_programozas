...
if triplets:
    ...
    print(f'Összes pitagorasz számhármas: {triplets}')
    print(f'A legnagyobb összegű c a {max_c[2]} és ez a {max_c} számhármasban van.')
else:
    print('Nincs ilyen számhármas.')
...
df = pd.read_csv('/home/g14/uni/sze_python_programozas/data/soccer.txt', sep=',', skiprows=1)
#print(df.info())
#print(df)

#1. Melyik bajnokságban ( country ) mennyi volt a mérkőzésenkénti gólátlag?
df['goals']=df['home_team_goal']+df['away_team_goal']
print(df.groupby('country')['goals'].mean().sort_values(ascending=False))
