...
df = pd.read_csv(
    '/home/g14/uni/sze_python_programozas/data/dog_breeds.txt', sep='|')

# print(df.info())

# 1. Melyik az 5 legnehezebb fajta a weight_low_lbs oszlop alapján?
print(df.groupby('Breed')['Weight_low_lbs'].max(
...
df['Height_high_inches'] = df['Height_high_inches'].apply(
    lambda x: x * 2.54)  # cm-re váltás
df['Height_low_inches'] = df['Height_low_inches'].apply(
    lambda x: x * 2.54)    # cm-re váltás
filtered_df = df[(df['Height_high_inches'] -
                  df['Height_low_inches']) >= 25]['Breed']
print(filtered_df)
...
max_weight_breed = shorter_breeds.loc[shorter_breeds['Weight_low_lbs'].idxmax(
)]['Breed']
